...
Definition dle_main_pcc_begin : Addr := A 9.
Definition dle_main_code_start : Addr := A 12.
Definition dle_main_pcc_end : Addr := A 67.
Definition dle_main_data_begin : Addr := A 74.
Definition dle_main_data_end : Addr := A 76.
Definition dle_main_exports_pcc : Addr := A 77.
Definition dle_main_exports_cgp : Addr := A 78.
Definition dle_main_exports_entries_begin : Addr := A 79.
Definition dle_main_exports_entries_end : Addr := A 79.

Definition dle_C_pcc_begin : Addr := A 67.
Definition dle_C_code_start : Addr := A 68.
Definition dle_C_pcc_end : Addr := A 74.
Definition dle_C_data_begin : Addr := A 76.
Definition dle_C_data_end : Addr := A 77.
Definition dle_C_exports_pcc : Addr := A 79.
Definition dle_C_exports_cgp : Addr := A 80.
Definition dle_C_exports_entries_begin : Addr := A 81.
Definition dle_C_exports_entries_end : Addr := A 82.

Definition dle_assert_begin : Addr := A 82.
Definition dle_assert_cap : Addr := A 94.
Definition dle_assert_end : Addr := A 95.
Definition dle_assert_flag : Addr := A 95.

Definition dle_switcher_begin : Addr := A 96.
Definition dle_switcher_call : Addr := A 97.
Definition dle_switcher_return : Addr := A 185.
Definition dle_switcher_end : Addr := A 247.
Definition dle_switcher_sealing_type : OType := OT 9.

Definition dle_trusted_stack_begin : Addr := A 4096.
Definition dle_trusted_stack_end : Addr := A 4196.
Definition dle_stack_begin : Addr := A 1024.
Definition dle_stack_end : Addr := A 1124.

Ltac unfold_dle_addresses :=
  unfold dle_main_pcc_begin, dle_main_code_start, dle_main_pcc_end,
    dle_main_data_begin, dle_main_data_end, dle_main_exports_pcc,
    dle_main_exports_cgp, dle_main_exports_entries_begin,
    dle_main_exports_entries_end, dle_C_pcc_begin, dle_C_code_start,
    dle_C_pcc_end, dle_C_data_begin, dle_C_data_end, dle_C_exports_pcc,
    dle_C_exports_cgp, dle_C_exports_entries_begin,
    dle_C_exports_entries_end, dle_assert_begin, dle_assert_cap,
    dle_assert_end, dle_assert_flag, dle_switcher_begin, dle_switcher_call,
    dle_switcher_return, dle_switcher_end, dle_switcher_sealing_type,
    dle_trusted_stack_begin, dle_trusted_stack_end, dle_stack_begin,
    dle_stack_end.
...
Definition dle_C_nested_read : instr.
Proof.
  constructor 5.
  - exact ct0.
  - exact ca0.
...
Definition dle_C_imports : list Word :=
  [WSentry XSRW_ Local dle_switcher_begin dle_switcher_end dle_switcher_call].
...
Proof.
  refine (@mkCmptSwitcher machine_parameters_instance
    dle_switcher_begin dle_switcher_end
    dle_switcher_call dle_switcher_return dle_switcher_sealing_type
    dle_trusted_stack_begin dle_trusted_stack_end
    _ _ _ _
    (replicate 100 (WInt 0)) _ eq_refl
    dle_stack_begin dle_stack_end (replicate 100 (WInt 0)) _ _).
...
Proof.
  refine (@mkCmptAssert machine_parameters_instance
    dle_assert_begin dle_assert_end dle_assert_cap dle_assert_flag _ _ _ _).
  - vm_compute; solve_addr.
  - vm_compute; solve_addr.
  - vm_compute; solve_addr.
  - unfold_dle_addresses.
...
Definition dle_C_f : Sealable :=
  SCap RO Global dle_C_exports_pcc dle_C_exports_entries_end
    dle_C_exports_entries_begin.
...
Proof.
  refine (@mkCmpt
    dle_main_pcc_begin dle_main_code_start dle_main_pcc_end
    dle_main_data_begin dle_main_data_end
    dle_main_exports_pcc dle_main_exports_cgp
    dle_main_exports_entries_begin dle_main_exports_entries_end
...
Proof.
  refine (@mkCmpt
    dle_C_pcc_begin dle_C_code_start dle_C_pcc_end
    dle_C_data_begin dle_C_data_end
    dle_C_exports_pcc dle_C_exports_cgp
    dle_C_exports_entries_begin dle_C_exports_entries_end
...
Definition dle_initial_registers : Reg :=
  <[PC := WCap RX Global dle_main_pcc_begin dle_main_pcc_end dle_main_code_start]>
  (<[cgp := WCap RW Global dle_main_data_begin dle_main_data_end
      dle_main_data_begin]>
  (<[csp := WCap RWL Local dle_stack_begin dle_stack_end dle_stack_begin]>
    (gset_to_gmap (WInt 0) all_registers_s))).

Definition dle_initial_sregisters : SReg :=
  <[MTDC := WCap RWL Local dle_trusted_stack_begin dle_trusted_stack_end
      dle_trusted_stack_begin]> ∅.
...
Theorem dle_concrete_adequacy reg' sreg' mem' es :
  rtc erased_step
