...
From griotte Require Import stack_object stack_object_spec_closure.
...
Section SO.
  ...
  Lemma so_init_spec
    ...
    ∗ inv (export_table_entryN SON (b_so_exp_tbl ^+ 2)%a)
      ...
      ∗ WSealed ot_switcher (SCap true RO Global b_so_exp_tbl e_so_exp_tbl (b_so_exp_tbl ^+ 2)%a) ↦□ₑ 2
  ...
  Proof.
    ...
    iApply (fetch_spec _ _ _ _ _ _ _ _ _
      ...
      with "[- $HPC $Hct0 $Hcs0 $Hcs1 $Hcode]"); eauto using switcher_call_sentry_not_heap.
    ...
    apply load_heap_nonheap in Hrcgp; [|rewrite /is_heap_cap /heap_cap_base /memory_cap_base /= Hcgp_nonheap; eauto].
