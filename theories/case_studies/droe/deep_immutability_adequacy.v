...
Section Adequacy.
  ...
  Proof.
    ...
    iEval (rewrite (elements_list_to_set [C] HCNoDup)) in "Hworld_interp_C".
    rewrite !big_sepL_singleton.
    set (W0 := (∅, (∅,∅), ∅, ∅)).

    pose proof (
        @droe_spec Σ ceriseg seal_storeg _ _ _ _ _ _ _ _ _ C
      ) as Spec.
    ...
    ; repeat (rewrite delete_insert_ne ; last (subst C_f ; intro ; simplify_eq; by rewrite H H0 H1 in Hneq_entries)).
    subst C_f; cbn.
    set (C_f := (SCap true RO Global (cmpt_exp_tbl_pcc C_cmpt) (cmpt_exp_tbl_entries_end C_cmpt)
                   (cmpt_exp_tbl_entries_start C_cmpt))).
    set (C_f' := (SCap true RO Local (cmpt_exp_tbl_pcc C_cmpt) (cmpt_exp_tbl_entries_end C_cmpt)
                   (cmpt_exp_tbl_entries_start C_cmpt))).
    ...
    assert ( (exported_entries_words C_cmpt) = {[WSealable C_f; borrow (WSealable C_f) ]}) as Hexported_entries_words.
    ...
    iMod (
       alloc_compartment_interp _ W0 C_cmpt C with "[HC_imports] [HC_code] [HC_data] [] [$Hworld_interp_C]"
      ) as "(Hworld_interp_C & #HC_code & #HC_data & _ & #HC_exports)"; eauto.
    ...
    { rewrite C_imports.
      ...
      - rewrite Hexported_entries_sealed Hexported_entries_words.
        ...
        iEval (rewrite fixpoint_interp1_eq /= /interp_sb).
        iSplit.
        { replace {[WSealable C_f'; borrow (WSealable C_f')]} with ({[borrow (WSealable C_f)]} : gset Word) by set_solver+.
    ...
    assert (Forall (λ a, is_heap_address a = false)
      (finz.seq_between (b_stack switcher_cmpt) (e_stack switcher_cmpt))) as Hstack_nonheap.
    ...
    assert (related_sts_priv_world W1 Winit_C) as Hrelated_W1_Winit_C.
    { subst Winit_C.
      apply related_sts_pub_priv_world.
      eapply related_sts_pub_update_multiple.
      eapply Forall_impl; [exact Hstack_disjoint|].
      intros a Ha; cbn in *.
    ...
    { rewrite Hexported_entries_sealed.
      ...
      { subst Winit_C. by rewrite std_update_multiple_heap. }
      { iPureIntro. exact Hrelated_W1_Winit_C. }
