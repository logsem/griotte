...
Section Adequacy.
  ...
  Proof.
    ...
    pose proof (
        @vae_init_spec Σ ceriseg seal_storeg _ _ _ _ _ _ _ _ _ C
      ) as Spec.
    ...
    set (C_g := (SCap true RO Global _ _ ((cmpt_exp_tbl_entries_start C_cmpt) ^+1)%a)).
    set (C_g' := (SCap true RO Local _ _ ((cmpt_exp_tbl_entries_start C_cmpt) ^+1)%a)).
    set (awk_f := (SCap true RO Global _ _ (cmpt_exp_tbl_pcc main_cmpt ^+ 2)%a)).
    set (awk_f' := (SCap true RO Local _ _ (cmpt_exp_tbl_pcc main_cmpt ^+ 2)%a)).
    ...
    iDestruct (world_interp_alloc_loc _ C false awk_rel_pub awk_rel_priv with "Hworld_C")
      as ">(Hworld_C & %Hloc_fresh & %Hcus_fresh & Hst_i & #Hrel_i)"; auto.
    ...
    set (awk'_f := (SCap true RO Global
                (cmpt_exp_tbl_pcc main_cmpt)
                (cmpt_exp_tbl_entries_end main_cmpt)
    ...
    iAssert (ot_switcher_prop W1' C (WSealable awk_f)) as "#ot_switcher_awk_f".
    {
      ...
      + solve_addr+H3 H4.
      + rewrite length_app.
        rewrite /vae_main_code !length_app in H4.
        solve_addr+H3 H4.
    }
    ...
    { by subst W0 W1. }
    iMod
      (world_interp_salloc W1 C ot_switcher_propC ot_switcher {[WSealable awk_f; WSealable awk_f' ]}
        with "[$Hsealed_pred_ot_switcher] [ ] [ ] [$Hworld_C]")
      as "(Hworld_C & Hseal_awk_f)"; first done.
    { iIntros (w); iApply mono_priv_ot_switcher. }
    { subst W1'.
      rewrite normalise_sealed_words_borrow.
      rewrite big_sepS_singleton; iFrame "ot_switcher_awk_f".
    }
    ...
    { iEval (rewrite fixpoint_interp1_eq /= /interp_sb).
      iSplit; first iFrame "Hseal_awk_f".
      iPureIntro; cbn.
    ...
    { rewrite C_imports.
      ...
      set ( Winter := (std_update_compartment W1' C_cmpt) ).
      ...
      - (* Switcher cross-compartment *)
        ...
        { iSplit.
          ...
          * iIntros (W2 W3) "!> %Hrelated %Hwf Hinterp".
            ...
            iApply (interp_in_mem_monotone_nl W2 W3 C RWL (WSealed ot_switcher awk_f)
                      with "Hinterp"); [exact Hwf|exact Hrelated|by unfold awk_f; cbn].
        }
    ...
    { rewrite Hexported_entries_sealed.
      ...
      ; first set_solver+.
      iApply (interp_monotone_sd_same_heap W2 Winit_C C with "[] [Hinterp_C_f]").
      { subst Winit_C. by rewrite std_update_multiple_heap. }
      { iPureIntro. exact Hrelated_pub_W2_Winit_C. }
    ...
    { eapply disjoint_from_shadow_not_in;
        first exact (cmpt_cgp_disjoint_from_shadow main_cmpt).
      pose proof (cmpt_data_size main_cmpt) as Hdata_size.
      rewrite main_data in Hdata_size.
      apply withinBounds_true_iff; solve_addr+Hdata_size. }
    { solve_ndisj. }
    { solve_ndisj. }
    { solve_ndisj. }
