...
Section cap_lang_rules.
  ...
  Proof.
    ...
    destruct HaStore as (?&?&?&?&?&?&Hrinr&Hwo&Hmem).
  ...
  Proof.
   ...
   Proof.
    ...
    destruct Hspec.
     ...
     { (* Failure (contradiction) *)
       ...
       ; try congruence.
       apply isCorrectPC_ra_wb in Hvpc. apply andb_prop_elim in Hvpc as [_ Hwb].
       destruct o; last apply Is_true_false in H0; try congruence ; try done.
     }
    Qed.
   ...
   Lemma wp_store_success_reg_PC_same E pc_p pc_g pc_b pc_e pc_a pc_a' w w' :
     ...
     (pc_a + 1)%a = Some pc_a' →
     writeAllowed pc_p = true →
     canStore pc_p (WCap pc_p pc_g pc_b pc_e pc_a) = true ->
     ...
     {{{ ▷ PC ↦ᵣ WCap pc_p pc_g pc_b pc_e pc_a
       ...
       {{{ RET NextIV;
           PC ↦ᵣ WCap pc_p pc_g pc_b pc_e pc_a'
              ∗ pc_a ↦ₐ WCap pc_p pc_g pc_b pc_e pc_a }}}.
   Proof.
     iIntros (Hinstr Hvpc Hpca' Hwa HcanStore φ)
            "(>HPC & >Hi) Hφ".
     iDestruct (map_of_regs_1 with "HPC") as "Hmap".
   ...
   (*         PC ↦ᵣ WCap pc_p pc_g pc_b pc_e pc_a' *)
   (*            ∗ pc_a ↦ₐ (if (a =? pc_a)%a *)
   (*                       then (WCap pc_p pc_g pc_b pc_e pc_a) *)
   (*                       else w) *)
   (*            ∗ dst ↦ᵣ WCap p g b e a *)
   (*            ∗ if (a =? pc_a)%a *)
   (*              then emp *)
   (*              else a ↦ₐ (WCap pc_p pc_g pc_b pc_e pc_a) }}}. *)
   (* Proof. *)
   (*   iIntros (Hinstr Hvpc Hpca' Hwa Hwb φ) *)
   (*          "(>HPC & >Hi & >Hdst & Ha) Hφ". *)
   (*   iDestruct (map_of_regs_2 with "HPC Hdst") as "[Hmap %]". *)
   ...
   (* Lemma wp_store_success_reg_frominstr_same E pc_p pc_g pc_b pc_e pc_a pc_a' w dst w' *)
   (*       p g b e : *)
   (*    decodeInstrW w = Store dst (inr PC) → *)
   (*   isCorrectPC (WCap pc_p pc_g pc_b pc_e pc_a) → *)
   (*   (pc_a + 1)%a = Some pc_a' → *)
  ...
  (*  Lemma wp_store_success_reg_frominstr E pc_p pc_g pc_b pc_e pc_a pc_a' w dst w' *)
  (*        p b e a : *)
  (*     decodeInstrW w = Store dst (inr PC) → *)
  (*    isCorrectPC (WCap pc_p pc_g pc_b pc_e pc_a) → *)
  (*    (pc_a + 1)%a = Some pc_a' → *)
  ...
  (*             ∗ pc_a ↦ₐ w *)
  (*             ∗ dst ↦ᵣ WCap p b e a *)
  (*             ∗ a ↦ₐ WCap pc_p pc_g pc_b pc_e pc_a }}}. *)
  (*  Proof. *)
  ...
  (* Qed. *)
   ...
   Lemma wp_store_fail_reg E pc_p pc_g pc_b pc_e pc_a pc_a' w dst src
         p g b e a w'' :
      decodeInstrW w = Store dst (inr src) →
     isCorrectPC (WCap pc_p pc_g pc_b pc_e pc_a) →
     (pc_a + 1)%a = Some pc_a' →
     writeAllowed p = true → withinBounds b e a = false →
     canStore p w'' = true ->
