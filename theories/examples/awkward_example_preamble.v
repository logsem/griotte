...
From cap_machine.examples Require Import region_macros macros
     stack_object_helpers awkward_example_helpers awkward_example_extra.
From cap_machine.examples Require Import awkward_example_u.
...
Section awkward_example_preamble.
  ...
  Definition awkward_preamble f_m offset_to_awkward ai :=
    ([∗ list] a_i;w_i ∈ ai;(awkward_preamble_instrs f_m offset_to_awkward), a_i ↦ₐ w_i)%I.
  ...
  Lemma awkward_preamble_spec (f_m f_a offset_to_awkward: Z) (r: Reg) W pc_p pc_b pc_e
    ...
    awkward_preamble f_m offset_to_awkward ai

    (* Code of the awkward example itself *)
    ∗ awkward_example ai_awk f_a r_adv
  ...
  Proof.
    ...
    iPrologue "Hprog".
    iApply (wp_move_success_reg with "[$HPC $Hi $Hr2 $Hr1]");
    ...
    iDestruct (sts_alloc_loc _ false awk_rel_pub awk_rel_pub awk_rel_priv with "Hsts") as ">HH".
    ...
    { rewrite /interp fixpoint_interp1_eq. iModIntro. rewrite /enter_cond.
      ...
      iApply (f4_spec with "[$HPC $Hradv $Hrenv $Hrstk $Hregs' $Hna $Hsts $Hr $Henv $Hawk $Hrel_i]");
        [apply Hvpc_awk|apply Hcont_awk|apply Hwb_assert|apply Ha_entry'|..].
      { auto. }
    ...
    { destruct g; iPureIntro. apply related_sts_priv_refl_world. apply related_sts_priv_refl_world.
