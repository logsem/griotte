...
Section AllocatorRules.
  ...
  (** A heap load consults the allocator invariant for this instruction only.
      The observed shadow bit determines whether the loaded tag is cleared. *)
  Lemma wp_load_heap_inv E r1 r2 pc_p pc_g pc_b pc_e pc_a w w'
    p g b e a (t' : bool) p' g' b' e' a' pc_a' dq dq' :
    ↑Nallocator ⊆ E →
  ...
  Proof.
    ...
    iDestruct (allocator_inv_lookup b' with "Hbody") as (s) "[[Hs Hres] Hput]".
    { by apply elem_of_heap_addresses. }
    destruct (shadow_bit s) eqn:Hbit.
    - iApply (wp_load_success_heap_revoked with "[$HPC $Hi $Hr1 $Hr2 $Ha $Hs]"); eauto.
      ...
      { iNext. iApply ("Hput" $! s). iFrame "Hres". by rewrite /allocator_entry Hbit. }
      iModIntro. iApply ("HΦ" $! true). iFrame.
    - iApply (wp_load_success_heap with "[$HPC $Hi $Hr1 $Hr2 $Ha $Hs]"); eauto.
      iNext. iIntros "(HPC & Hr1 & Hi & Hr2 & Ha & Hs)".
      iMod ("Hclose" with "[Hs Hres Hput]").
      { iNext. iApply ("Hput" $! s). iFrame "Hres". by rewrite /allocator_entry Hbit. }
      iModIntro. iApply ("HΦ" $! false). iFrame.
  Qed.
