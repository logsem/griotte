...
Section DROE.
  ...
  Lemma droe_spec
    ...
    (forall r, r ∈ (dom rmap ∖ dom_arg_rmap) -> rmap !! r = Some (WInt 0) ) ->
    (forall r, r ∈ (dom_arg_rmap) -> is_Some (rmap !! r) ) ->
  ...
  Proof.
    ...
    iIntros (HNswitcher_assert Hrmap_dom Hrmap_init Hrmap_arg_init HsubBounds
  ...
  Proof.
    ...
    iIntros (HNswitcher_assert HsubBounds
      ...
      "(#Hassert & #Hswitcher &#Hinterp_C_f)
      (HK & %Hframe_match & Hregister_state & Hrmap & HWreg_C & HWstd_full_C & Hcstk & Hna)".
    iDestruct "Hregister_state" as "(%Hfullrmap & %HPC & %Hcgp & %Hcra & Hcsp & Hinterp_rmap & %Hrmap_zero)".
    rewrite /interp_conf.
    ...
    iApply droe_spec; eauto ; last iFrame "∗#".
    ...
    { intros r Hr.
      ...
      apply Hrmap_zero.
      rewrite not_elem_of_union; split; set_solver.
    }
